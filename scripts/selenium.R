...
texto <- driver$client$findElement(using = 'id',
                                   'wc-textarea')
...
menu <- driver$client$findElement('class',
...
respuesta <- driver$client$findElement('class',
                                         'bm-webchat-text-entry-left')


respuestas <- driver$client$findElements('class',
                                         'bm-webchat-text-entry-left')

# respuestas tienen cada elemento de esa clase, luego tomamos el texto que contienen
texto_rptas <- lapply(respuestas,
                      FUN = function(x) {x$getElementText()})

# selenium permite así automatizar tareas complejas
# es menos rápido o eficiente que otros métodos pero facilita mucho el trabajo

driver$client$navigate("https://twitter.com/i/flow/login") #navegamos a urls 

inputCuenta <- driver$client$findElement(using = "css",
                                         value = ".r-30o5oe")

inputCuenta$highlightElement()

inputCuenta$clickElement()

inputCuenta$sendKeysToElement(list("cuentaejemplo23",
                             key = 'enter'))


pswCuenta <- driver$client$findElement(using = "css",
                                       value = ".r-homxoj")

pswCuenta$highlightElement()


pswCuenta$sendKeysToElement(list("Juano123456789!",
                             key = 'enter'))

tweets <- driver$client$findElements(using = "css",
                          value = "article")

texto_tweets <- lapply(tweets,
                      FUN = function(x) {x$getElementText()})


lupa <- driver$client$findElement(using = "css",
                                       value = "a.css-1dbjc4n:nth-child(2)")

lupa$highlightElement()

lupa$clickElement()

buscador <- driver$client$findElement(using = "css",
                                  value = ".r-30o5oe")

buscador$highlightElement()
buscador$clickElement()
buscador$sendKeysToElement(list("Massa",
                                key = 'enter'))

web <- driver$client$getPageSource()                              

html <- xml2::read_html(web[[1]])
